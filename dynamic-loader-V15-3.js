...
fetch(
  ...
  .then((data) => {
    ...
    lines.forEach((line, index) => {
      ...
      const tab = document.getElementById(`cms-tab-${index + 1}`);
      if (tab) {
        const innerDiv = tab.querySelector("[cms-content-list-url]");
        if (innerDiv) {
          innerDiv.setAttribute("cms-content-list-url", url);
          console.log(`✅ Updated tab ${index + 1} URL`);
        }
      }
    });
  })
