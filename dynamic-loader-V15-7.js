  ...
  if (tabs.every((tab) => tab !== null)) {
    function injectURL(line, idx) {
      ...
      if (tab && url) {
        ...
        if (contentDiv) {
          ...
          contentDiv.setAttribute("cms-content-list-count", "12"); // cap at 12 products
    ...
    fetch(sheetUrl)
      ...
      .then((data) => {
        ...
        // Split initial 3 and remaining tabs for staged loading
        ...
        // Load remaining tabs after 3 seconds
        setTimeout(() => {
          lines.forEach((line, idx) => {
            if (idx >= 3) {
              injectURL(line, idx);
